...
#define MIN_WIFI_RSSI -80   // Minimum acceptable signal
#define MAX_NETWORKS 10     // Maximum networks displayed
...
// Helper function to calculate credentials hash
...
String inputText(const String& prompt, int x, int y, bool isPassword = false) {
    ...
    while (true) {
        ...
        delay(10);  // Reduced for better responsiveness
    }
}
...
String scanAndDisplayNetworks() {
    // Configure async scan
    WiFi.scanDelete();
    WiFi.scanNetworks(true);
    
    M5Cardputer.Display.clear();
    M5Cardputer.Display.drawString("Searching networks...", 1, 1);
...
void connectToWiFi() {
    // Configure WiFi mode and power saving
    WiFi.mode(WIFI_STA);
    esp_wifi_set_ps(WIFI_PS_MAX_MODEM);
    
    // Try to recover saved credentials
    ...
    // If not connected, start configuration process
    M5Cardputer.Display.clear();
    M5Cardputer.Display.drawString("WiFi Configuration", 1, 1);
    ...
    M5Cardputer.Display.drawString("Enter password:", 1, 38);
    CFG_WIFI_PASS = inputText("> ", 4, M5Cardputer.Display.height() - 24, true);
    
    // Save credentials with hash
    preferences.begin(NVS_NAMESPACE, false);
    ...
    M5Cardputer.Display.clear();
    M5Cardputer.Display.drawString("Credentials saved.", 1, 60);
